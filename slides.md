...
OAuth 2.0が便利なので認証にも使いたい!ということで、OAuth 2.0を拡張して認証のためのアイデンティティレイヤーを提供するもの

1. そもそもなぜOAuth 2.0で認証をやってはいけないのか
2. OpenID Connect の基本用語
3. ID トークンの構造とクレーム
4. OpenID Connect のフロー
5. OpenID Connectの詳細
...
→認証の需要に応えるためにOAuth2.0ではカバーできない仕様を標準化した`OpenID Connect`が誕生。
...
- UserInfo エンドポイント: ユーザーのプロフィール情報を提供するエンドポイント
